...
def arrhenius(barrier_eV, temperature_K, prefactor_s):
    ...
    positive(barrier_eV, 'barrier', True)
    positive(temperature_K, 'temperature')
    positive(prefactor_s, 'prefactor', True)
...
def harmonic_free_energy(mode_energies_eV, temperature_K):
    ...
    return float(np.sum(0.5 * modes + kT * np.log(-np.expm1(-modes / kT))))


def eyring(delta_free_energy_eV, temperature_K, transmission=1.0):
    """Unimolecular same-standard-state TST rate; no gas pressure factor implied."""
    positive(temperature_K, 'temperature')
    if not math.isfinite(delta_free_energy_eV):
        raise ValueError('Free energy must be finite')
    if not math.isfinite(transmission) or not 0 <= transmission <= 1:
        raise ValueError('Classical transmission must lie in [0,1]')
    return (
        transmission
        * KB_EV
        * temperature_K
        / H_EV_S
        * math.exp(-delta_free_energy_eV / (KB_EV * temperature_K))
    )


def harmonic_tst(
    electronic_barrier_eV,
    temperature_K,
    is_modes_eV,
    ts_real_modes_eV,
    ts_imaginary_modes,
    ...
    fi = harmonic_free_energy(is_modes_eV, temperature_K)
    ft = harmonic_free_energy(ts_real_modes_eV, temperature_K)
    barrier = electronic_barrier_eV + ft - fi
    return dict(
        electronic_barrier_eV=electronic_barrier_eV,
        delta_vibrational_free_energy_eV=ft - fi,
        activation_Helmholtz_free_energy_eV=barrier,
        rate_s=eyring(barrier, temperature_K, transmission),
        qualified_for_kmc=False,
    )
...
def adsorption_hazard(pressure_Pa, gas_temperature_K, mass_amu, site_area_m2, sticking):
    ...
    positive(pressure_Pa, 'pressure', True)
    positive(gas_temperature_K, 'gas temperature')
    positive(mass_amu, 'mass')
    positive(site_area_m2, 'site area')
...
def isotropic_hop_diffusivity(
    per_neighbor_rate_s, hop_length_m, neighbors, dimension=2, correlation=1.0
):
    """D=f*z*k*l^2/(2*d); equivalent isotropic hops, rate is per neighbor."""
    positive(per_neighbor_rate_s, 'hop rate', True)
    positive(hop_length_m, 'hop length')
    if not isinstance(neighbors, int) or neighbors < 1 or dimension not in (1, 2, 3):
        raise ValueError('Invalid topology')
    if not math.isfinite(correlation) or not 0 <= correlation <= 1:
        raise ValueError('Correlation factor must lie in [0,1]')
    return correlation * neighbors * per_neighbor_rate_s * hop_length_m**2 / (2 * dimension)


def md_count_rate(event_count, reactive_site_time_s, confidence=0.95):
    ...
    return dict(
        rate_s=event_count / reactive_site_time_s,
        upper_rate_s=float(
            chi2.ppf(confidence, 2 * (event_count + 1)) / (2 * reactive_site_time_s)
        ),
