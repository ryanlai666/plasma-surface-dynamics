...
def fetch_hcl(root="data/raw/si_hcl"):
    ...
    write_json(root/"provenance.json", dict(source=api, doi=metadata["metadata"]["doi"],
               license="CC-BY-4.0", creators=metadata["metadata"]["creators"],
...
def inventory(archive, output):
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for member in z.infolist():
            if member.is_dir() or "__MACOSX" in member.filename or member.filename.split("/")[-1].startswith("."):
                continue
            ...
            if len(lines) >= 8:
                try:
                    ...
                    pass
            rows.append(dict(member=member.filename, bytes=member.file_size,
                             species=species, counts=counts,
                             sha256=hashlib.sha256(z.read(member)).hexdigest(),
                             source_doi="10.5281/zenodo.10211009"))
    write_csv(output, rows)
    return dict(files=len(rows), parsed_structures=sum(bool(r["species"]) for r in rows), output=str(output))
