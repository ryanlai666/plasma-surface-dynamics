...
def run(output="docs/mace_results"):
    ...
    if not cif.exists():
        ...
        with urllib.request.urlopen(
            "https://www.crystallography.net/cod/2102550.cif", timeout=60
        ) as r:
            cif.write_bytes(r.read())
    nitride = read(cif)
    assert nitride.get_chemical_formula() == "N8Si6", nitride.get_chemical_formula()
    structures = {"Si_diamond": bulk("Si", "diamond", a=5.43, cubic=True), "beta_Si3N4": nitride}
    ...
    for name, url in MODELS.items():
        ...
        for i, case in enumerate(cases):
            ...
            predictions.append(
                dict(
                    model=name,
                    case_id=i,
                    structure=case.info["case_name"],
                    strain=case.info["linear_strain"],
                    atoms=len(atoms),
                    n_si_ratio=symbols.count("N") / symbols.count("Si"),
                    energy_ev=energy,
            ...
            print(
                f"  {case.info['case_name']}, strain {case.info['linear_strain']:+.2f}: {energy / len(atoms):.6f} eV/atom",
                flush=True,
            )
        # Central finite difference at a displaced beta-Si3N4 configuration.
        probe = nitride.copy()
        probe.positions[0, 0] += 0.04
        probe.calc = calc
        ...
        checks.append(
            dict(
                model=name,
                analytic_force_ev_A=analytical,
                finite_difference_force_ev_A=numerical,
                abs_difference_ev_A=error,
                tolerance_ev_A=1e-4,
                passed=error < 1e-4,
            )
        )
    ...
    for i, case in enumerate(cases):
        disagreement.append(
            dict(
                ...
                structure=case.info["case_name"],
                strain=case.info["linear_strain"],
                force_component_rmse_between_models_ev_A=float(
                    np.sqrt(np.mean((forces["mp_0b2_small", i] - forces["mpa_0_medium", i]) ** 2))
                ),
            )
        )
    ...
    for ax, label in zip(axes.flat, structures):
        for name in MODELS:
            ...
            ax.plot(
                [r["strain"] * 100 for r in subset],
                [(r["energy_ev_per_atom"] - zero) * 1000 for r in subset],
                "o-",
                label=name,
            )
        ax.set(
            title=label, xlabel="Isotropic linear strain (%)", ylabel="E - E(unstrained) (meV/atom)"
        )
    ...
    report = dict(
        ...
        packages={
            ...
            for k in ["mace-torch", "torch", "ase", "e3nn", "numpy"]
        },
        device="cpu",
        threads=2,
        dtype="float64",
        ...
        structure_source="https://www.crystallography.net/cod/2102550.html",
        structure_license="CC0",
        ...
        limitations=[
            "No independent DFT energy/force labels: disagreement is not prediction error.",
