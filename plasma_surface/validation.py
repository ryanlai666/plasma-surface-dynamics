...
def run(output="docs/results"):
    ...
    tests = subprocess.run(
        [
            ...
            "-p",
            "no:cacheprovider",
            f"--basetemp=outputs/pytest-campaign-{uuid.uuid4().hex}",
        ],
        cwd=root,
        capture_output=True,
        text=True,
        ...
        for sites in sizes:
            ...
            convergence.append(
                dict(
                    ...
                    mean_nm=mean,
                    std_nm=std,
                    standard_error_nm=float(se),
                    reference_nm=target,
                    absolute_bias_nm=abs(mean - target),
                    within_five_se=bool(abs(mean - target) <= 5 * se + 1e-12),
                )
            )
            print(
                f"  {backend}, {sites} sites: bias={abs(mean - target):.3g} nm, SE={se:.3g} nm",
                flush=True,
            )
    ...
    saturation = [
        dict(dose_s=float(dose), cycle10_epc_nm=epc(model.ale_recipe(28, float(dose), 20)))
        for dose in [0, 0.1, 0.25, 0.5, 1, 2, 4, 8, 16, 32]
    ]
    ...
    axes[1].set(
        xlabel="Modification duration (s)",
        ylabel="Cycle-10 EPC (nm/cycle)",
        title="28 eV; removal duration 20 s",
    )
    ...
    if archive.exists():
        ...
        public.update(
            ...
            note="Geometry inventory is not etch-rate validation",
        )

    checks = dict(
        backend_agreement=max_backend_error < 1e-12,
        stochastic_ensembles=all(r["within_five_se"] for r in convergence),
        chemistry_only_zero=all(
            r["cycle10_epc_nm"] == 0 for r in controls if r["control"] == "chemistry_only"
        ),
        below_threshold_zero=all(
            r["cycle10_epc_nm"] == 0 for r in controls if r["energy_ev"] == 10
        ),
        dose_saturates=abs(saturation[-1]["cycle10_epc_nm"] - saturation[-2]["cycle10_epc_nm"])
        < 1e-5,
    )
    report = dict(
        ...
        reference_five_cycle_removal_nm=target,
        convergence=convergence,
        surrogate=metrics,
        speedup=timing["wall_time_speedup"],
        public_data=public,
    ...
    run_manifest = manifest(
        dict(
            ...
            samples=4096,
            parameters=asdict(p),
            phases=[asdict(s) for s in phases],
            reference_cycles=5,
            cpp_ensemble_repeats=128,
            python_ensemble_repeats=32,
        )
    )
    run_manifest["validation_driver_sha256"] = hashlib.sha256(
    ...
    (output / "REPORT.md").write_text(
        f"""# Simulation and numerical verification report
