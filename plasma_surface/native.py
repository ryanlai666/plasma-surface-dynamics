...
def simulate(phases, p=None, cycles=5, *, method="kmc", sites=256, seed=42, max_events=2_000_000):
    ...
    inputs = np.ascontiguousarray(
        [[phase.duration_s, *rates(phase, p)] for phase in phases], dtype=np.float64
    )
    ...
    if status:
        raise RuntimeError(
            {
                1: "Invalid native simulation inputs",
                2: "Event budget exceeded",
                3: "Native allocation or runtime failure",
            }.get(status, "Unknown native error")
        )
    rows = []
    for i, values in enumerate(outputs):
        row = dict(
            cycle=i // len(phases) + 1,
            phase=phases[i % len(phases)].name,
            **dict(
                zip(
                    ["time_s", "coverage", "removed_nm", "deposited_nm", "net_removed_nm"],
                    map(float, values[:5]),
                )
            ),
        )
