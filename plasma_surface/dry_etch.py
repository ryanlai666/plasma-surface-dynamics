...
def f2_rate(law, temperature_K, pressure_Pa):
    ...
    return dict(k_m3_s=k, impact_probability=gamma, density_m3=density, hazard_s=k * density)
...
def main():
    ...
    for law in laws:
        ...
        for t, r in zip(temps, rates):
            records.append(
                dict(facet=law['facet'], temperature_K=float(t), pressure_Pa=pressure, **r)
            )
        rate = f2_rate(law, 500.0, pressure)['hazard_s']
        ...
        axs[1].plot(times[1:], expected[1:], '--', lw=0.8, color='black', alpha=0.4)
    axs[0].set(
        xlabel='Surface/gas temperature (K)',
        ylabel='First-reaction hazard per site (1/s)',
        title='Published F2/Si rate law at 1 Torr F2',
    )
    axs[1].set(
        xlabel='Exposure time (s)',
        ylabel='Fraction of initial sites reacted',
        title='500 K: Gillespie ensemble and exact expectation',
        xscale='log',
        ylim=(0, 1.02),
    )
    for ax in axs:
        ax.grid(alpha=0.2)
        ax.legend(title='Si facet')
    fig.text(
        0.5,
        0.01,
        'First F2 reaction on susceptible F-terminated sites; no layer removal, ion bombardment or ALE cycling implied.',
        ha='center',
        fontsize=9,
    )
    fig.tight_layout(rect=[0, 0.04, 1, 1])
    fig.savefig(out / 'f2_surface_kinetics.png', dpi=160)
    plt.close(fig)
    ...
    with (ROOT / 'data/literature/sin_hf_pathways.csv').open() as f:
            ...
            for nu in (1e11, 1e12, 1e13):
                sensitivity.append(
                    dict(
                        ...
                        barrier_eV=ea,
                        conditional_hazard_s=nu * math.exp(-ea / (KB_EV * t)),
    ...
    with (out / 'sin_hf_sensitivity.csv').open('w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=list(sensitivity[0]))
        w.writeheader()
        w.writerows(sensitivity)
    manifest = dict(
        status='passed' if all(c['passed'] for c in checks) else 'failed',
        source=json.loads(source.read_text())['source'],
        ...
        f2_rate_rows=len(records),
        hf_sensitivity_rows=len(sensitivity),
    )
    (out / 'verification.json').write_text(json.dumps(manifest, indent=2) + '\n')
    print(json.dumps(manifest, indent=2))
    if manifest['status'] != 'passed':
        raise RuntimeError('Kinetic verification failed')
