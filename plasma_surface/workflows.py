...
def campaign(n=128, seed=42, shard=0, shards=1, backend="python"):
    ...
    points = qmc.scale(qmc.LatinHypercube(4, seed=seed).random(n),
                       [5, 0.1, 0.1, 280], [90, 6, 6, 500])
    rows = []
    for i, x in enumerate(points):
        ...
        rows.append(dict(recipe_id=i, **dict(zip(FEATURES, map(float, x))),
                         epc_nm=float(epc), data_origin="synthetic_uncalibrated_model"))
    return rows
...
def train_surrogate(rows, output, seed=42):
    ...
    low = x[:, 0] < 65
    extrapolation = ExtraTreesRegressor(n_estimators=160, min_samples_leaf=2, random_state=seed, n_jobs=1)
    extrapolation.fit(x[low], y[low])
    metrics = dict(mae_nm=float(mean_absolute_error(y[test], prediction)),
                   rmse_nm=float(np.sqrt(mean_squared_error(y[test], prediction))),
                   constant_baseline_mae_nm=float(mean_absolute_error(y[test], np.full(len(test), y[train].mean()))),
                   high_energy_holdout_mae_nm=float(mean_absolute_error(y[~low], extrapolation.predict(x[~low]))),
                   train_recipe_ids=groups[train].tolist(), test_recipe_ids=groups[test].tolist(),
    ...
    write_csv(Path(output)/"surrogate_predictions.csv", [dict(recipe_id=int(groups[i]),
              true_epc_nm=float(y[i]), predicted_epc_nm=float(pred), tree_spread_nm=float(std))
...
def demo(output, n=128, seed=42, backend="python"):
    ...
    for data, label in [(rom, "Mean field"), (stochastic, "kMC (512 sites)")]:
        axes[0].plot([r["time_s"] for r in data], [r["net_removed_nm"] for r in data], ".-", label=label)
    ...
    plt.close(fig)
    write_json(output/"manifest.json", manifest(dict(parameters=asdict(p), phases=[asdict(x) for x in phases],
                                                   n=n, seed=seed, cycles=5, kmc_sites=512, backend=backend)))
    return metrics
...
def calibrate(csv_path, output):
    ...
    values = np.array([[float(r[k]) for k in FEATURES+["cycle", "epc_nm", "sigma_nm"]] for r in rows])
    ...
    def residual(x):
        ...
        predictions = []
        for row in rows:
            trace = mean_field(ale_recipe(*(float(row[f]) for f in FEATURES)), p, int(float(row["cycle"])))
    ...
    result = least_squares(residual, [.25, .8], bounds=([0, 0], [1, 5]))
    report = dict(sticking=float(result.x[0]), chemical_yield_scale=float(result.x[1]),
                  success=bool(result.success), weighted_residual_sum_squares=float(2*result.cost),
