...
def main():
    ...
    src=ROOT/'data/final_cleavage/SiF3_NH2_HF/omol25/IS.extxyz';out=src.parent.parent/'local_coordinate_scan';out.mkdir(exist_ok=False)
    ...
    for direction,seq in [('forward',list(enumerate(targets))),('reverse',list(enumerate(targets))[::-1])]:
        for i,(sin,nhdist) in seq:
            ...
            physical=a.get_forces(apply_constraint=False);projected=a.get_forces();b=freeze(a);b.info.update(calculation_kind='Evaluated relaxed two-coordinate scan; not a minimum-energy path or TS',direction=direction,target_SiN_A=float(sin),target_NH_A=float(nhdist));frames.append(b)
            rows.append(dict(direction=direction,index=i,energy_eV=float(a.get_potential_energy()),target_SiN_A=float(sin),target_NH_A=float(nhdist),SiN_A=float(a.get_distance(0,4)),NH_A=float(a.get_distance(4,8)),SiF_A=float(a.get_distance(0,7)),HF_A=float(a.get_distance(7,8)),projected_force_eV_A=float(np.linalg.norm(projected,axis=1).max()),mobile_physical_force_eV_A=float(np.linalg.norm(physical[4:],axis=1).max()),converged=ok))
            write(out/'evaluated_geometries.extxyz',frames);(out/'summary.json').write_text(json.dumps(dict(runner='scripts/scan_final_cleavage_coordinates.py',runner_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),source_sha256=hashlib.sha256(src.read_bytes()).hexdigest(),model_sha256=hashlib.sha256(args.checkpoint.read_bytes()).hexdigest(),status='complete' if len(rows)==22 else 'running',scope='Constrained local Si-N elongation and proton transfer; forward/reverse hysteresis diagnostic; no TS or kinetic barrier',rate_enabled=False,results=rows),indent=2)+'\n');print(direction,i,rows[-1],flush=True)
