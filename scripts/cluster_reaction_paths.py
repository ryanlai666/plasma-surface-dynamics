...
def main():
    ...
    out = (
        ROOT / 'data/surface_paths' / (args.motif + '_capped_motif') / args.reactant / 'omol25_neb'
    )
    ...
    record = dict(
        ...
        runner='scripts/cluster_reaction_paths.py',
    ...
    opt = BFGS(neb, logfile=str(out / 'neb.log'), maxstep=0.08)
    opt.attach(lambda: write(out / 'checkpoint.extxyz', images), interval=5)
    opt.run(fmax=0.12, steps=80)
    neb.climb = True
    ok = bool(opt.run(fmax=0.025, steps=220))
