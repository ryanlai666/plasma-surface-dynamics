...
def main():
    ...
    rows = json.loads((out / 'relaxed_adsorption.json').read_text())
    for r in rows:
        for key in ['source', 'folder']:
            r[key] = r[key].replace(chr(92), '/')
    ...
    for ax, surface in zip(axes.flat, surfaces):
        for r in [r for r in rows if r['surface'] == surface]:
            ...
            data = list(csv.DictReader((folder / 'energies.csv').open()))
            steps = [int(x['step']) for x in data]
            es = np.array([float(x['energy_eV']) for x in data])
            ax.plot(
                steps,
                es - es[0],
                label=f"Start {r['start']}: "
                + ('converged' if r['converged'] else 'not converged'),
            )
            reference = json.loads((folder.parent / 'summary.json').read_text())['bare_slab']
            ads = (
                'not reported'
                if r['adsorption_energy_eV'] is None
                else f"{r['adsorption_energy_eV']:.3f}"
            )
            table.append(
                f"| {surface} / {r['start']} | {r['site']} / {r['orientation']} | {'yes' if r['converged'] else 'no'} / {'yes' if reference['converged'] else 'no'} | {r['max_mobile_force_eV_A']:.4f} | {r['relaxation_energy_eV']:.3f} | {ads} | {r['HF_distance_A']:.3f} | [data + GIF](../../{r['folder']}/README.md) |"
            )
            if not (folder / 'relaxation.gif').exists():
                images = read(folder / 'trajectory.extxyz', ':')
                indices = np.unique(
                    np.linspace(0, len(images) - 1, min(12, len(images)), dtype=int)
                )
                frames = []
                ...
                for i in indices:
                    a = images[i]
                    f = plt.figure(figsize=(9, 4.5), dpi=100)
                    view = f.add_subplot(121, projection='3d')
                    ep = f.add_subplot(122)
                    view.scatter(
                        *a.positions.T,
                        s=[35 if x == 'H' else 75 for x in a.get_chemical_symbols()],
                        c=[COLORS[x] for x in a.get_chemical_symbols()],
                        edgecolors='#394955',
                        linewidths=0.4,
                    )
                    for setter, c in zip([view.set_xlim, view.set_ylim, view.set_zlim], mid):
                        setter(c - span * 0.55, c + span * 0.55)
                    view.set_box_aspect([1, 1, 1])
                    view.view_init(25, -55)
                    view.axis('off')
                    view.set_title(surface + ' / HF')
                    ep.plot(steps, es - es[0], color='#288A91')
                    ep.scatter(i, es[i] - es[0], color='#D15A30', s=60)
                    ep.set(
                        xlabel='FIRE optimization step',
                        ylabel='Energy relative to start (eV)',
                        title=f'Actual evaluated step {i}',
                    )
                    ep.grid(alpha=0.2)
                    f.suptitle(
                        'Upper substrate + HF relaxation; lower substrate fixed', fontsize=13
                    )
                    f.text(
                        0.5,
                        0.03,
                        'Optimization trajectory, not a reaction coordinate or transition-state path. No interpolated frames.',
                        ha='center',
                        fontsize=9,
                    )
                    f.subplots_adjust(bottom=0.19, top=0.81, wspace=0.05)
                    b = io.BytesIO()
                    ...
                    json.dumps(
                        dict(
                            ...
                            gif_sha256=hashlib.sha256(
                                (folder / 'relaxation.gif').read_bytes()
                            ).hexdigest(),
                        ),
                        indent=2,
                    )
...
"""
    ...
    for r in audit:
        text += f"| {r['surface']} / {r['start']} | {r['frozen_fragment_interaction_eV']:.3f} | {r['substrate_reference_shift_eV']:.3f} | {r['molecular_layer_reference_shift_eV']:.3f} | {'yes' if r['lower_clean_slab_geometry_found'] else 'not flagged'} |\n"
    ...
    (out / 'RELAXED_ADSORPTION.md').write_text(text, encoding='utf-8')
    print('Reported and rendered', len(rows), 'relaxed starts')
