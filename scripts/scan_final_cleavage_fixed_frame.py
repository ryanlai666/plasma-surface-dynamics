...
def main():
    ...
    out = src.parent.parent / 'fixed_frame_three_coordinate_scan'
    ...
    targets = list(
        zip(
            np.linspace(start, 3.3, 11),
            np.linspace(nh, 1.025, 11),
            np.linspace(a.get_distance(0, 7), 1.60, 11),
        )
    )
    ...
    for direction, seq in [
        ...
        for i, (sin, nhdist, sif) in seq:
            ...
            a.set_constraint(
                FixedFrameDistances([0, 1, 2, 3], [(0, 4), (4, 8), (0, 7)], [sin, nhdist, sif])
            )
            a.set_positions(a.positions.copy())
            ...
            assert np.max(abs(a.positions[:4] - fixed_xyz)) < 1e-10
            b = freeze(a)
            b.set_constraint(FixAtoms(indices=[0, 1, 2, 3]))
            b.info.update(
                calculation_kind='Evaluated relaxed three-coordinate scan; not a minimum-energy path or TS',
                direction=direction,
                target_SiN_A=float(sin),
                target_NH_A=float(nhdist),
                target_SiF_A=float(sif),
            )
            frames.append(b)
            rows.append(
                dict(
            ...
            (out / 'summary.json').write_text(
                json.dumps(
                    dict(
                        runner='scripts/scan_final_cleavage_fixed_frame.py',
                        runner_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                        source_sha256=hashlib.sha256(src.read_bytes()).hexdigest(),
                        constraint_sha256=hashlib.sha256(
                            (ROOT / 'scripts/fixed_frame_constraints.py').read_bytes()
                        ).hexdigest(),
                        model_sha256=hashlib.sha256(args.checkpoint.read_bytes()).hexdigest(),
                        status='complete' if len(rows) == 22 else 'running',
                        scope='Fixed-frame constrained Si-N elongation, proton transfer and Si-F approach; forward/reverse hysteresis diagnostic; no TS or kinetic barrier',
                        rate_enabled=False,
                        results=rows,
                    ),
                    indent=2,
