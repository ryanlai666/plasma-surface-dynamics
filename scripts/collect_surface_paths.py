...
def main():
    ...
    parser.add_argument('--archive',type=Path,default=ROOT/'data/raw/sicl4_surface/supplement.zip')
    ...
    with zipfile.ZipFile(args.archive) as z:pdf=z.read('Supporting Information.pdf')
    doc=pymupdf.open(stream=pdf,filetype='pdf');text='\n'.join(p.get_text() for p in doc)
    parts=re.split(r'^\d+\. ([^\n]+)\n',text,flags=re.M)
    inventory=[]
    for i in range(1,len(parts),2):
        name=parts[i].strip();key=name.replace('Si(100) surface.','clean_surface')
        rows=re.findall(r'\b(Si|Cl|H)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)',parts[i+1])
        symbols=[r[0] for r in rows];xyz=np.array([[float(v) for v in r[1:]] for r in rows])
        count=112 if key=='clean_surface' else 5 if key=='STC' else 117
        if len(rows)!=count or not np.isfinite(xyz).all():raise ValueError('Incomplete coordinates: '+name)
        a=Atoms(symbols,positions=xyz);a.info.update(source_doi='10.3390/sym15010213',source_label=name,coordinates_only=True,cell_not_supplied=True)
        path=target/(key+'.extxyz');write(path,a)
        inventory.append(dict(label=key,natoms=len(a),formula=a.get_chemical_formula(),sha256=hashlib.sha256(path.read_bytes()).hexdigest()))
    if len(inventory)!=17:raise ValueError('Expected 17 source structures')
