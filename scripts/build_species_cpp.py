...
info = dict(command=command, compiler=subprocess.check_output([compiler,"--version"], text=True).splitlines()[0],
            platform=platform.platform(), source_sha256=hashlib.sha256((ROOT/"cpp/species.cpp").read_bytes()).hexdigest(),
            binary_sha256=hashlib.sha256(target.read_bytes()).hexdigest())
(directory/"species_build_manifest.json").write_text(json.dumps(info, indent=2)+"\n", encoding="utf-8")
print(target)
