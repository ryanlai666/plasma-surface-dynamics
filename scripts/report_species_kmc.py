...
for r in d['temperatures']:
    g = r['products_per_initial_motif']
    lines.append(
        f"| {r['temperature_K']:.0f} | {r['Si_released_fraction']:.4f} | {g['SiF4']:.4f} | {g['SiH2F2']:.4f} | {g['SiHF3']:.4f} | {g['NH3']:.4f} |"
    )
lines += [
    '',
    ...
    f"- Maximum ensemble discrepancy from the master equation: {max(x['max_standard_errors'] for x in d['verification']):.2f} estimated standard errors; acceptance threshold 6.",
    f"- Independent Python/C++ ensembles: maximum {d['python_cpp_max_standard_errors']:.2f} estimated standard errors in the checked populated states.",
    '',
...
for name, r in d['benchmark'].items():
    lines.append(f"| {name} | {r['median_seconds']:.6f} | {r['events_per_second']:.0f} |")
lines += [
    '',
...
p = out / 'REPORT.md'
p.write_text(
    p.read_text(encoding='utf-8-sig').split('<!-- RESULTS -->')[0]
    + '<!-- RESULTS -->\n\n'
    + '\n'.join(lines),
    encoding='utf8',
)
