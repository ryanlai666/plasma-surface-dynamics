...
def curvature(atoms, calc, mobile, delta=0.005):
    a = atoms.copy()
...
def run(surface_name, species, calc, checkpoint):
    ...
    near = [
        ...
        if a.symbol == preferred and i != site and a.position[2] > base.positions[:, 2].max() - 2.2
    ]
    vectors = {
        i: find_mic(base.positions[i] - base.positions[site], base.cell, base.pbc)[0] for i in near
    }
    neighbor = min(near, key=lambda i: np.linalg.norm(vectors[i]))
    target = base.positions[site] + vectors[neighbor]
    away = vectors[neighbor].copy()
    away[2] = 0
    ...
    second_height = (
        (1.52 if preferred == 'Si' else 1.05)
        if gas[breaking].symbol == 'H'
        else (
            2.12 if gas[breaking].symbol == 'Cl' else 1.75 if gas[breaking].symbol == 'F' else 1.48
        )
    )
    destination = target + [0, 0, second_height] + away * 0.35
    ...
    record = dict(
        ...
        substrate_atoms=n,
        active_surface_pair=[site, neighbor],
        source=parent['source'],
        proposed_reaction=f'{species} + reactive surface pair -> dissociated adsorbates',
        breaking_bond_indices=[n, n + breaking],
        gas_bond_length_A=bond0,
        ...
        runner='scripts/run_molecular_reaction_neb.py',
        script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        limitations=[
            'Ideal unpassivated cleavage, one coverage and site; no slab convergence study',
    ...
    for label, a in [('IS', initial), ('FS', final)]:
        ...
        opt = BFGS(a, logfile=str(out / (label + '.log')), maxstep=0.1)
        ok = bool(opt.run(fmax=0.02, steps=220))
        saved = snapshot(a)
        write(out / (label + '.extxyz'), saved)
        ...
        record[label] = dict(
            ...
            no_negative_curvature=bool(eig.min() > -0.02),
        )
        save()
    displacement = find_mic(endpoints[1].positions - endpoints[0].positions, base.cell, base.pbc)[0]
    record['endpoint_mobile_RMS_displacement_A'] = float(
        np.sqrt(np.mean(displacement[mobile] ** 2))
    )
    if not all(record[k]['converged'] and record[k]['no_negative_curvature'] for k in ['IS', 'FS']):
        ...
        save()
        print(surface_name, species, record['status'], flush=True)
        return
    if abs(record['FS']['breaking_bond_A'] - record['IS']['breaking_bond_A']) < 0.35:
        record['status'] = 'no_distinct_dissociation_endpoints'
        record['seconds'] = time.perf_counter() - start
        save()
        print(surface_name, species, record['status'], flush=True)
        return
    ...
    for a in images:
        a.calc = independent(calc)
    neb = NEB(images, k=0.25, climb=False, method='improvedtangent')
    neb.interpolate(method='idpp', apply_constraint=True)
    opt = BFGS(neb, logfile=str(out / 'neb.log'), maxstep=0.1)
    opt.attach(lambda: write(out / 'checkpoint.extxyz', images), interval=5)
    opt.run(fmax=0.15, steps=80)
    neb.climb = True
    ok = bool(opt.run(fmax=0.035, steps=180))
    ...
    peak = int(es.argmax())
    xyz = np.array([a.positions for a in saved])
    arc = np.r_[
    ...
    rows = [
        dict(
            ...
            energy_eV=float(es[i]),
            relative_energy_eV=float(es[i] - es[0]),
            breaking_bond_A=float(a.get_distance(n, n + breaking, mic=True)),
            surface_pair_distance_A=float(a.get_distance(site, neighbor, mic=True)),
        )
    ...
    if 0 < peak < len(images) - 1:
        ...
        if ok and record['index_one_in_mobile_subspace']:
            ...
            for sign in [-1, 1]:
                ...
                a.calc = independent(calc)
                opt = BFGS(a, logfile=str(out / f'downhill_{sign}.log'), maxstep=0.1)
                done = bool(opt.run(fmax=0.02, steps=180))
                ...
                errors = [
                    float(
                        np.sqrt(
                            np.mean(
                                find_mic(
                                    a.positions[mobile] - b.positions[mobile], base.cell, base.pbc
                                )[0]
                                ** 2
                            )
                        )
            ...
            record['endpoint_connectivity_confirmed'] = sorted(matched) == [0, 1] and all(
                c['converged'] for c in connections
            )
    record['seconds'] = time.perf_counter() - start
    save()
    print(surface_name, species, record['status'], record.get('peak_above_IS_eV'), flush=True)
