...
def energy_snapshot(atoms):
    ...
    for key in batch.keys:
        value=batch[key]
        if isinstance(value,torch.Tensor) and value.is_floating_point():batch[key]=value.to(dtype=dtype)
    ...
    result=atoms.copy();result.calc=SinglePointCalculator(result,energy=energy)
    return result
...
def main():
    ...
    for surface in SURFACES:
        for species in MOLECULES:
            ...
            root=parent/'mace_site_orientation';root.mkdir(exist_ok=True)
            base.calc=independent(calc);e_slab=float(base.get_potential_energy())
            rows=[]
            if (root/'all_energies.csv').exists():continue
            for site_name,site_position,site_atoms in sites(base):
                ...
                for orientation in orientations:
                    ...
                    path=root/site_name/orientation;path.mkdir(parents=True,exist_ok=True)
                    if (path/'summary.json').exists():
                        with (path/'energies.csv').open() as handle:
                            rows.extend(dict(site=site_name,orientation=orientation,**r) for r in csv.DictReader(handle))
                        continue
                    point_rows=[];images=[]
                    for n,height in enumerate(HEIGHTS):
                        ...
                        image=energy_snapshot(a);images.append(image);e=float(image.get_potential_energy())
                    ...
                    meta=dict(surface=surface,species=species,site=site_name,site_position_A=site_position.tolist(),site_atom_indices=site_atoms,
                              ...
                              runner='scripts/screen_molecular_sites_energy.py',energy_only=True,script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest())
