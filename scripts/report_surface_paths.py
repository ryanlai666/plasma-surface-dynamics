...
def main():
    rows=[]; galleries=[]; summary_rows=[]
    for p in sorted((ROOT/'data/surface_paths').glob('*/*/mace_neb/summary.json')):
        s=json.loads(p.read_text());v=json.loads(p.with_name('curvature.json').read_text());m=json.loads(p.with_name('second_model.json').read_text());rel=p.parent.relative_to(ROOT).as_posix()
        state='constrained saddle checked' if v['eligible_constrained_saddle'] else 'candidate only; see checks'
        rows.append(f"| {s['surface']} / {s['species']} | {s['reaction']} | 0 | {s['peak_above_IS_eV']:.4f} | {s['FS_minus_IS_eV']:+.6f} | {s['max_neb_force_eV_A']:.4f} | {state} |")
        summary_rows.append(f"| {s['surface']} / {s['species']} migration | {s['peak_above_IS_eV']:.4f} | MACE CI-NEB; {s['images']} evaluated images; {state} |")
        galleries.append(f"### {s['surface']} / {s['species']}\n\n![{s['species']} surface migration with calculated image energies](../../{rel}/path.gif)\n\n[Coordinates, energies and checks](../../{rel}/README.md). MPA-0 single-point sampled peak on this path: **{m['sampled_peak_above_IS_eV']:.4f} eV**; this is a model-sensitivity result, not an independent TS calculation.\n")
    if len(rows)!=3:raise ValueError('Three completed path folders required')
    table='| Surface / species | Reaction | IS (eV) | Peak above IS (eV) | FS minus IS (eV) | Max NEB force (eV/A) | Validation |\n|---|---|---:|---:|---:|---:|---|\n'+'\n'.join(rows)
    p=ROOT/'docs/dry_etch_results/SURFACE_PATHS.md';s=p.read_text(encoding='utf8');start='<!-- CALCULATED_TABLE -->';end='<!-- END CALCULATED_TABLE -->'
    content=start+'\n\n'+table+'\n\n'+'\n'.join(galleries)+'\n'+end
    if end in s:s=s[:s.index(start)]+content+s[s.index(end)+len(end):]
    else:s=s.replace(start,content)
    p.write_text(s,encoding='utf8')
    p=ROOT/'README.md';s=p.read_text(encoding='utf8');a=s.index('### Surface paths: calculated images and energies') if '### Surface paths: calculated images and energies' in s else s.index('### Surface IS -> TS -> FS animation');b=s.index('### Literature-parameterized dry-etch kinetics',a)
    replacement=r"""### Surface paths: calculated images and energies
...
"""
    s=s[:a]+replacement+s[b:];p.write_text(s,encoding='utf8')
