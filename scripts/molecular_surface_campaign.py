...
def molecule(name):
    diatomics = {'HF': ('FH', .92), 'HCl': ('ClH', 1.28),
                 'F2': ('F2', 1.42), 'Cl2': ('Cl2', 1.99), 'H2': ('H2', .75)}
    if name in diatomics:
        symbols, bond = diatomics[name]
        return Atoms(symbols, positions=[[0, 0, 0], [0, 0, bond]])
    if name == 'H2O':
        return Atoms('OH2', positions=[[0, 0, 0], [.757, 0, .586], [-.757, 0, .586]])
    angles = np.arange(3) * 2 * np.pi / 3
    if name == 'CH3F':
        xyz = [[0, 0, 0], [0, 0, 1.38]]
        xyz += [[1.09 * np.sqrt(8/9) * np.cos(a), 1.09 * np.sqrt(8/9) * np.sin(a), 1.38+1.09/3] for a in angles]
        return Atoms('FCH3', positions=xyz)
    bond = 1.56 if name == 'SiF4' else 2.02
    xyz = [[0, 0, 0], [0, 0, -bond]]
    xyz += [[bond*np.sqrt(8/9)*np.cos(a), bond*np.sqrt(8/9)*np.sin(a), bond/3] for a in angles]
...
def slab(name):
    if name in ('Si100', 'Si111'):
        ...
        cif = ROOT / ('docs/mace_results/2102550.cif' if name.startswith('beta') else 'data/reference/surface_bulk/9005018.cif')
        bulk = read(cif)
        atoms = surface(bulk, (0, 0, 1), 2, vacuum=12)
        if name.startswith('alpha'):
            atoms = atoms.repeat((2, 1, 1))
        source = {'bulk_cif': str(cif.relative_to(ROOT)), 'bulk_sha256': hashlib.sha256(cif.read_bytes()).hexdigest(),
                  'source_url': 'https://www.crystallography.net/cod/' + ('2102550' if name.startswith('beta') else '9005018') + '.html',
                  'construction': 'Unrelaxed (001) cleavage, two unit-cell layers; quartz doubled along a',
                  'bulk_temperature_note': 'Quartz source structure measured at 398 K; this is not a finite-temperature simulation' if name.startswith('alpha') else 'Public beta-Si3N4 structure'}
...
def main():
    ...
    for name in args.species:
        ...
        opt = BFGS(a, logfile=None, maxstep=.08)
        ok = bool(opt.run(fmax=.01, steps=150))
        if not ok:
    ...
    for surface_name in args.surfaces:
        ...
        si = [i for i, atom in enumerate(base) if atom.symbol == 'Si' and atom.position[2] > top-2.0]
        ...
        for name in args.species:
            ...
            for n, height in enumerate(HEIGHTS):
                ...
                a.calc = independent(calc)
                a.info.update(surface=surface_name, species=name, height_A=height, substrate_atoms=len(base),
                ...
                rows.append(dict(image=n,nearest_molecular_atom_height_A=height,energy_eV=e,
            ...
            meta = dict(surface=surface_name, species=name, images=len(images), substrate_atoms=len(base),
                        formula=images[0].get_chemical_formula(), surface_site_index=site, source=source,
                        energy_reference='E(slab+molecule) - E(slab) - E(periodic molecular layer), same cell and fixed geometries',
                        slab_energy_eV=e_slab, molecular_layer_energy_eV=e_gas,
                        ...
                        model='MACE-MP-0b2 small', model_sha256=hashlib.sha256(checkpoint.read_bytes()).hexdigest(),
                        runner='scripts/molecular_surface_campaign.py', script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                        versions={p: importlib.metadata.version(p) for p in ['ase','mace-torch','numpy','torch']},
                        seconds=time.perf_counter()-start, TS_status='Not searched: scan maxima are NOT transition states',
                        assumptions=['Ideal unpassivated rigid surface; no reconstruction, defects or thermal motion',
            ...
            print(surface_name, name, 'sampled minimum', minimum['interaction_energy_eV'], flush=True)
