...
images = read(out / 'input.extxyz', ':')
saved = read(out / 'images.extxyz', ':')
rows = meta['frames']
meta['continuations'] = [
    dict(
        runner='scripts/resume_dft_singlepoints.py',
        script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        start_image=len(rows),
        reason='Direct SCF failed at this image; use Newton fallback without changing geometry or functional/basis',
    )
]
for i in range(len(rows), len(images)):
    ...
    rows.append(
        dict(
            image=i,
            energy_eV=energy,
            relative_energy_eV=energy - rows[0]['energy_eV'],
    ...
    meta['electronic_state_note'] = (
        'Converged restricted-singlet SCF; no proof of lowest electronic or spin state'
    )
    (out / 'summary.json').write_text(json.dumps(meta, indent=2) + '\n')
