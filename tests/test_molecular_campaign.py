...
def test_site_orientation_matrix_preserves_geometries_and_energy_references(surface):
    ...
    for species in MOLECULES:
        ...
        for path in paths:
            ...
            rows = list(csv.DictReader((p / 'energies.csv').open()))
            assert len(fs) == len(rows) == 9 and s['peak_is_TS'] is False
            assert matches_recorded(ROOT / s['runner'], s['script_sha256'])
            for (meta, symbols, xyz), r in zip(fs, rows):
                ...
                assert molecular[:, 2].min() - base[:, 2].max() == pytest.approx(
                    float(r['height_A']), abs=2e-8
                )
                assert np.allclose(
                    molecular[:, :2].mean(axis=0), s['site_position_A'][:2], atol=2e-8
                )
                assert float(meta['energy']) == pytest.approx(float(r['energy_eV']), abs=1e-7)
                reference = (
                    float(r['energy_eV']) - s['slab_energy_eV'] - s['molecular_layer_energy_eV']
                )
...
def test_reaction_labels_do_not_promote_failed_searches_to_verified_ts():
    ...
    for p in summaries:
        ...
        assert s['status'] != 'endpoint_search'
        assert matches_recorded(ROOT / s['runner'], s['script_sha256'])
        ...
        if 'peak_above_IS_eV' in s:
            rows = list(csv.DictReader((p.parent / 'energies.csv').open()))
...
def test_direct_dft_results_preserve_geometry_and_reject_unresolved_root():
    ...
    for p in paths:
        ...
        assert matches_recorded(ROOT / s['runner'], s['script_sha256'])
        assert matches_recorded(p.parent / 'input.extxyz', s['input_sha256'])
        if p.parents[2].name == 'SiO_capped_motif' and p.parent.name == 'dft_path':
            ...
            assert matches_recorded(ROOT / s['SCF_audit_runner'], s['SCF_audit_script_sha256'])
        before = frames(p.parent / 'input.extxyz')
        after = frames(p.parent / 'images.extxyz')
...
def test_energy_only_evaluator_matches_independent_regular_evaluations():
    ...
    assert matches_recorded(ROOT / d['runner'], d['script_sha256'])
...
def test_refined_reaction_artifacts_and_ts_classification():
    ...
    for p in paths:
        ...
        assert d['status'] in ['neb_converged', 'neb_unconverged']
        assert matches_recorded(ROOT / d['runner'], d['script_sha256'])
        fs = frames(p.parent / 'images.extxyz')
        rows = list(csv.DictReader((p.parent / 'energies.csv').open()))
        ...
        if d.get('endpoint_connectivity_confirmed'):
            ...
            assert d['max_neb_force_eV_A'] <= 0.035
            assert sum(x < -0.02 for x in d['peak_hessian_eigenvalues_eV_A2']) == 1
...
def test_local_saddle_uses_the_minima_actually_connected():
    ...
    assert d['status'] == 'constrained_saddle_connected'
    assert (
        d['endpoint_connectivity_confirmed'] and not d['original_precursor_connectivity_confirmed']
    )
    assert all(
        d[k]['converged'] and min(d[k]['hessian_eigenvalues_eV_A2']) > 0 for k in ['IS', 'FS']
    )
    assert sum(x < -0.02 for x in d['peak_hessian_eigenvalues_eV_A2']) == 1
    fs = frames(p / 'images.extxyz')
    es = np.array([float(f[0]['energy']) for f in fs])
    assert d['peak_image'] == int(es.argmax()) and d['peak_above_IS_eV'] == pytest.approx(
        es.max() - es[0], abs=1e-8
    )
    audit = json.loads((p / 'identity_audit.json').read_text())
    assert 'N25' in audit['event'] and 'N26' in audit['event']
    ...
    assert matches_recorded(p / 'images.extxyz', audit['path_sha256'])
    assert matches_recorded(ROOT / d['runner'], d['script_sha256'])
...
def test_relaxed_adsorption_moves_substrate_and_preserves_fixed_atoms(surface):
    ...
    for r in d['starts']:
        p = ROOT / Path(r['folder'].replace(chr(92), '/'))
        fs = frames(p / 'trajectory.extxyz')
        fixed = r['fixed_indices']
        assert len(fs) == r['frames'] and len(fs) == r['steps'] + 1
        assert matches_recorded(
            (ROOT / Path(r['source'].replace(chr(92), '/'))), r['source_sha256']
        )
        for f in fs:
            assert np.array_equal(f[2][fixed], fs[0][2][fixed])
        ...
        if r['converged'] and d['bare_slab']['converged']:
            assert r['adsorption_energy_eV'] == pytest.approx(
                r['energy_eV'] - d['bare_slab']['energy_eV'] - d['gas_energy_eV']
            )
        else:
            assert r['adsorption_energy_eV'] is None
        m = json.loads((p / 'render_manifest.json').read_text())
        assert m['interpolated_frames'] == 0
        assert m['frame_indices'][0] == 0 and m['frame_indices'][-1] == len(fs) - 1
        assert matches_recorded(p / 'trajectory.extxyz', m['trajectory_sha256'])


def test_adsorption_decomposition_detects_unstable_reference_comparison():
    rows = json.loads((ROOT / 'docs/dry_etch_results/adsorption_reference_audit.json').read_text())
    assert len(rows) == 8
    for r in rows:
        assert abs(r['closure_error_eV']) < 1e-10
        assert matches_recorded(ROOT / r['runner'], r['runner_sha256'])
        folder = (
            ROOT
            / 'data/surface_paths'
            / r['surface']
        ...
        es = float(frames(folder / 'frozen_slab.extxyz')[0][0]['energy'])
        em = float(frames(folder / 'frozen_molecular_layer.extxyz')[0][0]['energy'])
        ec = float(frames(folder / 'trajectory.extxyz')[-1][0]['energy'])
