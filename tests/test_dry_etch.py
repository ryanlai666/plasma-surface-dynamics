...
LAWS = json.loads(
    (Path(__file__).resolve().parents[1] / 'data/literature/si_f2_kinetics.json').read_text()
)['laws']
...
def test_source_reference_temperature_and_pressure_units():
    law = LAWS[0]
    t = 298.15
    p = 133.32236842105263
    r = f2_rate(law, t, p)
    expected_k = 8.82e-21 * math.exp(-0.13 / (KB_EV * t))
...
def test_gillespie_first_event_conservation_and_limits():
    ...
    x = first_event_kmc(2, 50, [0, 0.01, 0.1, 1, 100], 1)
...
def test_published_path_barriers_obey_same_saddle_reference():
    d = json.loads(
        (
            Path(__file__).resolve().parents[1] / 'data/literature/sicl4_surface_paths.json'
        ).read_text()
    )
    for r in d['paths']:
        assert r['barrier_forward_eV'] - r['barrier_reverse_eV'] == pytest.approx(
            r['reaction_energy_eV']
        )
