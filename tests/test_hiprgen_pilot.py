...
def test_native_bucket_output_and_candidate_conservation():
    p = ROOT / 'data/reaction_network/hiprgen'
    d = json.loads((p / 'network.json').read_text())
    ...
    for f in d['families']:
        ...
        with sqlite3.connect(p / (f['family'] + '_buckets.sqlite')) as con:
            assert con.execute('SELECT count(*) FROM complexes').fetchone()[0] == 17 + 17 * 18 // 2
        for r in (r for r in d['reactions'] if r['family'] == f['family']):
            sides = []
            for side in ['reactants', 'products']:
                total = Counter()
                for name in r[side]:
                    total.update(entries[name]['composition'])
                sides.append(total)
    ...
    for name, sha in d['source_sha256'].items():
        assert matches_recorded(ROOT / 'third_party/hiprgen_snapshot' / name, sha)
