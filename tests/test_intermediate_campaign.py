...
def xyz(path):
    lines=path.read_text().splitlines();n=int(lines[0]);return lines[1],np.array([[float(x) for x in l.split()[1:4]] for l in lines[2:n+2]]),Counter(l.split()[0] for l in lines[2:n+2])
...
def test_coadsorbate_provenance_stoichiometry_and_reference_energies():
    ...
    for key in ('runner','helper','refinement_runner'):
        assert hashlib.sha256((ROOT/d[key]).read_bytes()).hexdigest()==d[key+'_sha256']
    assert len(d['results'])==8
    for r in d['results']:
        folder=ROOT/r['folder'];head,pos,composition=xyz(folder/'final.extxyz')
        source_head,source_pos,parent_composition=xyz(ROOT/r['source'])
        expected=parent_composition+Counter({'F':1,'H':1} if r['coadsorbate']=='HF' else {'O':1,'H':2})
        assert composition==expected
        assert hashlib.sha256((folder/'final.extxyz').read_bytes()).hexdigest()==r['final_sha256']
        assert hashlib.sha256((folder/'trajectory.extxyz').read_bytes()).hexdigest()==r['trajectory_sha256']
        ...
        assert r['incremental_association_energy_eV']==pytest.approx(energy-r['reference']['energy_eV']-r['gas_reference']['energy_eV'])
...
def test_candidate_network_retains_negative_curvature_and_has_no_fabricated_rates():
    ...
    assert any(n['curvature_screen']['minimum_eigenvalue_eV_A2']<-.02 for n in d['nodes'])
    for n in d['nodes']:
        assert len(n['symbols'])==len(n['positions_A'])
        assert all(max(i,j)<len(n['symbols']) for i,j in n['distance_graph'])
        assert hashlib.sha256((ROOT/n['structure']).read_bytes()).hexdigest()==n['structure_sha256']
    assert all(e['rate_s'] is None and e['barrier_eV'] is None and not e['rate_enabled'] for e in d['edges'])
