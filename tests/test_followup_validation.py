...
def test_full_host_stability_excludes_unstable_references():
    ...
    for r in list(d['parents'].values()) + d['results']:
        assert matches_recorded(ROOT / r['final_structure'], r['final_sha256'])
    ...
    for r in q['results']:
        if (
            r['parent_minimum_curvature_eV_A2'] < -0.02
            or r['candidate_minimum_curvature_eV_A2'] < -0.02
        ):
...
def test_direct_dft_diagnostics_remain_excluded_from_rates():
    ...
    for r in d['results']:
        ...
        assert r['frames'][0]['max_mobile_DFT_force_eV_A'] > 0.02
    for name in ('omol25', 'omol25_bfgs'):
        r = json.loads((base / name / 'summary.json').read_text())
        assert r['status'] == 'neb_unconverged' and not r['rate_enabled']
...
def test_corrected_scan_frame_and_invalid_run_are_distinguished():
    ...
    def frames(path):
        ...
        while i < len(lines):
            ...
            result.append(
                np.array(
                    [[float(x) for x in line.split()[1:4]] for line in lines[i + 2 : i + 2 + n]]
                )
            )
            i += n + 2
        return result
