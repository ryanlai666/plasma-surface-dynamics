...
def test_observation_preserves_seeded_trajectory_and_copies_states():
    ...
    assert -snapshots[-1]['heights'].mean()*Parameters().layer_nm==pytest.approx(actual[-1]['net_removed_nm'])
...
def test_uniform_sampling_keeps_idle_and_endpoints():
    history,snapshots=sample_trajectory([Phase('idle',1)],Parameters(),cycles=1,side=4,frames=11)
