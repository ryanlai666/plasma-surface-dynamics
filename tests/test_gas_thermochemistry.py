...
def test_source_table_values():
    ...
    assert h['Cp_J_mol_K'] == pytest.approx(29.18, abs=0.01)
    assert h['S_standard_J_mol_K'] == pytest.approx(139.2, abs=0.1)
    w = gas_thermo('H2O', 500, 100000)
    assert w['Cp_J_mol_K'] == pytest.approx(35.22, abs=0.01)
    assert w['H_minus_H298_kJ_mol'] == pytest.approx(6.92, abs=0.01)
...
def test_thermodynamic_derivatives_and_pressure(species):
    ...
    assert (plus['H_minus_H298_kJ_mol'] - minus['H_minus_H298_kJ_mol']) / (2 * d) == pytest.approx(
        center['Cp_J_mol_K'] / 1000, rel=1e-7
    )
    low = gas_thermo(species, T, 100000 / math.e)
    assert center['mu_minus_H298_eV'] - low['mu_minus_H298_eV'] == pytest.approx(
        R_J_MOL_K * T / 1000 / KJ_MOL_PER_EV
    )
