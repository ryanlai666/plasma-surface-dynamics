...
pytestmark = pytest.mark.skipif(
    not native.library_path().is_file(), reason="Optional C++ backend not built"
)
...
@pytest.mark.parametrize(
    ...
    [
        model.ale_recipe(),
        model.ale_recipe(75, 0.5, 3, 450),
        [model.Phase("growth", 2, precursor_flux_m2_s=1e19)],
        [
            model.Phase(
                "mixed",
                2,
                radical_flux_m2_s=2e19,
                ion_flux_m2_s=2e19,
                ion_energy_ev=60,
                precursor_flux_m2_s=1e19,
            )
        ],
        [model.Phase("idle", 2)],
    ],
...
def test_native_kmc_ensemble_matches_exact_solution(phases):
    ...
    for field in ["coverage", "net_removed_nm", "removed_nm", "deposited_nm"]:
        ...
        assert (
            abs(values.mean() - target[field])
            < 5 * values.std(ddof=1) / np.sqrt(len(values)) + 1e-6
        )
    for row in result:
