...
def test_kmc_ensemble_matches_mean_field():
    ...
    for name in ["net_removed_nm", "coverage"]:
        ...
        assert abs(values.mean()-reference[name]) < 5*values.std(ddof=1)/np.sqrt(len(values)) + 1e-5
...
@pytest.mark.parametrize("kwargs", [{"temperature_k": 0}, {"ion_energy_ev": -1}, {"duration_s": float("nan")}])
...
def test_campaign_shards_match_serial():
    serial = campaign(32)
    parallel = sorted(campaign(32, shard=0, shards=2)+campaign(32, shard=1, shards=2), key=lambda r:r["recipe_id"])
...
def test_calibration_recovers_synthetic_parameters(tmp_path):
    p = replace(Parameters(), sticking=.38, chemical_yield_scale=1.1)
    rows = []
    for dose in [.3, 1, 3]:
        for energy in [22, 35, 60]:
            trace = mean_field(ale_recipe(energy, dose), p, cycles=2)
            rows.append(dict(energy_ev=energy, dose_s=dose, ion_s=2, temperature_k=300,
                             cycle=2, epc_nm=trace[-1]["net_removed_nm"]-trace[-5]["net_removed_nm"],
                             sigma_nm=.001, source_id="synthetic_parameter_recovery_test"))
    path = tmp_path/"measurements.csv"
    write_csv(path, rows)
    fit = calibrate(path, tmp_path/"fit.json")
    assert fit["success"]
    assert fit["jacobian_rank"] == 2
    assert fit["sticking"] == pytest.approx(.38, abs=1e-5)
