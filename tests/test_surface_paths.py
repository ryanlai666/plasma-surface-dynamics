...
def frames(path):
    ...
    while i<len(lines):
        n=int(lines[i]);meta=dict(t.split('=',1) for t in shlex.split(lines[i+1]) if '=' in t)
        atoms=[line.split() for line in lines[i+2:i+2+n]]
        result.append((meta,[r[0] for r in atoms],np.array([[float(v) for v in r[1:4]] for r in atoms])))
        i+=n+2
    return result
...
def test_evaluated_path_energy_and_constraints(surface,species):
    ...
    s=json.loads((p/'summary.json').read_text());v=json.loads((p/'curvature.json').read_text()); fs=frames(p/'images.extxyz')
    with (p/'energies.csv').open() as f:rows=list(csv.DictReader(f))
    ...
    assert s['status']=='converged' and s['max_neb_force_eV_A']<=.025
    assert v['re_evaluation_max_energy_error_eV']<1e-5
    assert v['endpoint_minima_in_mobile_subspace']
    assert s['script_sha256']==hashlib.sha256((ROOT/s['command'][0]).read_bytes()).hexdigest()

@pytest.mark.parametrize('surface,species',SYSTEMS)
def test_animation_has_only_calculated_frames(surface,species):
    p=ROOT/'data/surface_paths'/surface/species/'mace_neb';m=json.loads((p/'render_manifest.json').read_text())
    with Image.open(p/'path.gif') as im:assert im.n_frames==m['frame_count']
    assert m['frame_image_indices']==list(range(m['frame_count'])) and m['interpolated_frames']==0
    for file,key in [('energies.csv','energies_sha256'),('images.extxyz','coordinates_sha256'),('path.gif','gif_sha256')]:
...
def test_source_slideshows_contain_only_three_stationary_points():
    ...
    for r in manifest['paths']:
        with Image.open(p/(r['path'].lower()+'.gif')) as im:assert im.n_frames==3
