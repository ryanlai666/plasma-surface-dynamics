...
spec=importlib.util.spec_from_file_location('reaction_check',ROOT/'scripts/check_reaction_catalog.py')
...
def test_rejects_missing_hydrogen_product():
    ...
    with pytest.raises(ValueError,match='Unbalanced'):module.validate(catalog)


def test_rejects_charge_imbalance_even_with_atom_balance():
    catalog=dict(species={'neutral':dict(composition={'Ar':1},charge=0),
